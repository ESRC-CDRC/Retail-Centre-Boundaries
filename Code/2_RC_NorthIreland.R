...
###
##  NORTHERN IRELAND RETAIL CENTRE DELINEATION
##   
##  - Code for delineating the first iteration of retail agglomerations for Northern Ireland.
...
##   
##  - Requires: OSM retail points and land use (NI_OSM_amenity/ shop/ LU.gpkg)
...
# READ IN OSM DATA 
#------------------------------------------------------------------------------------
osm.shopPT <- st_read(dsn = paste0(wd, "/Data/OSM/NI_OSM_shop_291020.gpkg"))
osm.shopPT$value <- NA
osm.amenityPT <- st_read(dsn = paste0(wd, "/Data/OSM/NI_OSM_amenity_291020.gpkg"))
...
osm.retailLU <- st_read(dsn = paste0(wd, "/Data/OSM/NI_OSM_LU_291020.gpkg"))
#------------------------------------------------------------------------------------
...
rm(tt, osm.retailLUTR, H3_osm.retailLU.11, H3_osm.retailLU.13, H3_osm.retailLU.13.A, H3_osm.retailLU.13.B, cl)
...
hex_Graph <- get.tractID(retail_centres$h3_address, retail_centres$retailN, retail.count=TRUE, NAME="RC_NI", source=retail_centres$source)
...
st_write(RC.dissolved, dsn = paste0(wd, "/Exports/Interim/NI_RC_Complete.gpkg"), delete_layer = TRUE, driver = "gpkg")
st_write(retail_centres, dsn = paste0(wd, "/Exports/Interim/NI_H3_Complete.gpkg"), delete_layer = TRUE, driver = "gpkg")
#------------------------------------------------------------------------------------